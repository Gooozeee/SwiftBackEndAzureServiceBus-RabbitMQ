...
def run_mediapipe_pose(filename):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            #print('key-points: ',str(results.pose_landmarks))

            # plot 3d graph of the landmarks per frame
...
def run_script(filename):
    ...
    #left_wrist_keypoints, right_wrist_keypoints = run_mediapipe_pose(filename)
    ...
    if not filename.endswith('-reduced-frames.avi'):
        ...
        # remove larger video for more efficient storage
